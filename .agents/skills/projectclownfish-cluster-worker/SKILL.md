...
- Prepare the target repo toolchain before the agentic edit/review loop. For OpenClaw this means Node 22+, Corepack, the target `packageManager`, and `pnpm install --frozen-lockfile`; only set `CLOWNFISH_INSTALL_TARGET_DEPS=0` for debugging failed setup.
- Review failed worker artifacts before requeueing. The workflow must upload worker artifacts even when `review-results` fails; a missing artifact after a failed review is a workflow bug, not an acceptable blind retry.

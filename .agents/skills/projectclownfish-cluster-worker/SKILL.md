...
- Fix execution should provide Codex actual repo-discovery context before editing; repeated "no target repo changes" means tune `scripts/execute-fix-artifact.mjs` before replaying more jobs. GitHub Actions may block Codex bwrap write sandboxes, so write-mode fix execution defaults to `danger-full-access` there after GH tokens are stripped from the Codex environment.

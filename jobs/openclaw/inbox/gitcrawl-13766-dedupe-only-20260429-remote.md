---
repo: openclaw/openclaw
cluster_id: gitcrawl-13766-dedupe-only-20260429-remote
mode: autonomous
allowed_actions:
  - comment
  - label
  - close
blocked_actions:
  - force_push
  - bypass_checks
  - merge
  - fix
  - raise_pr
require_human_for:
  - security_sensitive
  - failing_checks
  - conflicting_prs
  - unclear_canonical
  - broad_code_delta
canonical:
  - "#52751"
candidates:
  - "#43238"
  - "#50755"
  - "#52751"
  - "#53821"
  - "#59170"
  - "#69988"
cluster_refs:
  - "#43238"
  - "#50755"
  - "#52751"
  - "#52775"
  - "#53821"
  - "#59170"
  - "#60807"
  - "#69988"
security_policy: central_security_only
security_sensitive: false
allow_instant_close: true
allow_fix_pr: false
allow_merge: false
allow_post_merge_close: false
require_fix_before_close: false
canonical_hint: "gitcrawl representative #52751 is open; worker must verify it is still the best live canonical."
notes: "Generated from gitcrawl run cluster 13766 on 2026-04-29."
---

# Gitcrawl Cluster 13766

Generated from local gitcrawl run cluster 13766 for `openclaw/openclaw`.

Display title:

> feat: add per-agent compaction and contextPruning config overrides

Cluster shape from gitcrawl:

- total members: 8
- issues: 1
- pull requests: 7
- open candidates in local store: 6
- representative: #52751, currently open in local store
- latest member update: 2026-04-28T21:55:43.539524702Z

## Goal

Run one live autonomous classification pass. Classify open candidates only, verify live GitHub state, choose the current canonical issue or PR if the representative is obsolete, and emit only high-confidence planned close/comment/label actions. Closed context refs are evidence only and must not receive close actions.

## Member Inventory

Closed context refs:

- #52775 feat: per-agent session maintenance config overrides
- #60807 feat: per-agent compaction configuration overrides

Open candidates:

- #43238 feat(context-pruning): add rolling pruning mode
- #50755 feat: per-agent session maintenance config overrides
- #52751 feat: add per-agent compaction and contextPruning config overrides
- #53821 Add per-agent sandbox session visibility override feature
- #59170 feat: per-agent session visibility and agentToAgent scoping
- #69988 feat(config): support per-agent compaction overrides
